...
router.get('/', function(req, res, next) {
	function findID(data, idToLookFor) {
		// based on the uid of a friend, find their data in members and return
		// it
	...
	fs.readFile('data/members.json', 'utf8', function(err, data) {
		if (err) {
			...
			for (i = 0; i < friends.length; i++) {
				// for each friendID in array push data for that member to an
				// array to send to the client
